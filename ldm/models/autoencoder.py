...
class AutoencoderKL(pl.LightningModule):
    def __init__(self,
        ...
        if use_vf is not None:
            if use_vf == "dinov2":
                ...
                raise ValueError(f"Unknown foundation model: {use_vf}")
            
            if reverse_proj:
                self.proj = torch.nn.Linear(self.embed_dim, self.foundation_model.num_features)
            else:
                self.proj = torch.nn.Linear(self.foundation_model.num_features, self.embed_dim)
    ...
    def rescale(self, x, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
        x = (x + 1.0) / 2.0
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            aeloss, log_dict_ae = self.loss(inputs, reconstructions, posterior, z_prime, features, optimizer_idx, self.global_step,
                                            last_layer=self.get_last_layer(), encoder_last_layer=self.get_encoder_last_layer(), split="train")
        ...
        if optimizer_idx == 1:
            ...
            discloss, log_dict_disc = self.loss(inputs, reconstructions, posterior, z_prime, features, optimizer_idx, self.global_step,
                                                last_layer=self.get_last_layer(), encoder_last_layer=self.get_encoder_last_layer(), split="train")
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.use_vf is not None:
            ...
            if self.reverse_proj:
                ...
                features = self.proj(features)
                z_prime = z.flatten(2).transpose(1, 2)
        aeloss, log_dict_ae = self.loss(inputs, reconstructions, posterior, z_prime, features, 0, self.global_step,
                                        last_layer=self.get_last_layer(), encoder_last_layer=self.get_encoder_last_layer(), split="val")

        discloss, log_dict_disc = self.loss(inputs, reconstructions, posterior, z_prime, features, 1, self.global_step,
                                            last_layer=self.get_last_layer(), encoder_last_layer=self.get_encoder_last_layer(), split="val")
